...
class POJO(object):
    ...
    def toStringTeacher(self):
        print("Class Pojo [disciplineteacher = " + self.disciplineteacher + ", countdiscipline = " + self.countdiscipline + ", countstudent = " + self.countstudent + ", fullname = " + self.fullname + "]")


if __name__ == '__main__':
    # создаем экземпляр класса POJO
    p = POJO()
    ...
    # выводим полученный POJO
    print("\nПолученный POJO класс: ")
    p.toStringDiscipline()
    p.toStringTeacher()
    print("*" * 35)

    print("\nПроводим сериаллизацию")
    ...
    # изменяем значение тега weekday
    p.get_weekday = 'Суббота'

    # выводим класс POJO с обновлённым аргументом
    print("\nОбновленный POJO с изменённым аргументом: ")
    p.toStringDiscipline()
    p.toStringTeacher()
    print("*" * 35)

    # выгружаем объект из файла и выводим на экран
